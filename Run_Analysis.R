...
#Load in to R all the datasets 
YTest <- read.table("/Users/francescomancino/Desktop/R-programming/UCI HAR Dataset/test/y_test.txt")

XTest <- read.table("/Users/francescomancino/Desktop/R-programming/UCI HAR Dataset/test/x_test.txt")
SubjectTest <- read.table("/Users/francescomancino/Desktop/R-programming/UCI HAR Dataset/test/subject_test.txt")

YTrain <- read.table("/Users/francescomancino/Desktop/R-programming/UCI HAR Dataset/train/y_train.txt")

XTrain <- read.table("/Users/francescomancino/Desktop/R-programming/UCI HAR Dataset/train/x_train.txt")

SubjectTrain <- read.table("/Users/francescomancino/Desktop/R-programming/UCI HAR Dataset/train/subject_train.txt")

Features<- read.table("/Users/francescomancino/Desktop/R-programming/UCI HAR Dataset/features.txt")

#Include the name columns in the XTrain and XTest datasets
colnames(XTrain) <- t(Features[2])
colnames(XTest) <- t(Features[2])

#Include Columns from Y and SubjectTrain datasets to X train and Test datasets
XTrain$activities <- YTrain[, 1]
XTrain$participants <- SubjectTrain[, 1]
XTest$activities <- YTest[, 1]
XTest$participants <- SubjectTest[, 1]

#Assignment 1 Task: Merges the training and the test sets to create one data set

Master <- rbind(XTrain, XTest)
duplicated(colnames(Master))
Master <- Master[, !duplicated(colnames(Master))]

#Assignment 2 Task: Extracts only the measurements on the mean and standard deviation for each measurement.
...
#Assignment 3 Task: Uses descriptive activity names to name the activities in the data set
...
#Assignment 4 Task: Appropriately labels the data set with descriptive variable names
...
#Assignment 5 Task: Create a tidy data set
